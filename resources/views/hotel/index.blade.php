                ...
                <div class="box">
                    <div class="box-header">
                        ...
                        <div class="pull-left box-tools">
                            <a href="{{ route('hotel.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;&nbsp;Tambah Hotel</a>
                        </div>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        ...
                        <table id="datatable-standard" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @php $no = 1; @endphp
                                @forelse ($daftarHotel as $row)
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>{{ $row->nama }} </td>
                                        <td>{{ $row->lokasi }} </td>
                                        <td>
                                            <form action="{{ route('hotel.destroy', $row->id) }}" method="POST" onsubmit="return confirmDelete()">
                                                @csrf
                                                <input type="hidden" name="_method" value="DELETE">
                                                <a href="{{ route('hotel.edit', $row->id) }}" class="btn btn-warning btn-sm" title="Edit"><i class="fa fa-edit"></i></a>
