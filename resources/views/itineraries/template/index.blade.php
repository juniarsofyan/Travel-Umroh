            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">List Template Itinerary</h3>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        ...
                        <table id="datatable-standard" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @php $no = 1; @endphp
                                @forelse ($itineraryTemplates as $row)

                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>{{ $row->kode_template }} </td>
                                        <td>{{ $row->jumlah_hari }} </td>
                                        <td>{{ $row->detail_template_count }} </td>
                                        <td>
                                            <form action="{{ route('itinerary-templates.destroy', $row->id) }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="_method" value="DELETE">
                                                <a href="{{ route('itinerary-templates.edit', $row->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>
